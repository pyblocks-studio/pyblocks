...
- Python import uses the clearly named, lossless **Import Python as Raw Code** path instead of pretending to provide incomplete AST conversion.
...
| A-05–A-06 | Renamed import honestly, preserve raw source losslessly, group it as one undo action, confirm replacement, and restore the prior workspace if block creation fails. |
...
2. Python import is deliberately lossless raw source, not AST-to-visual-block conversion. This follows option A from the audit and avoids semantic loss.
