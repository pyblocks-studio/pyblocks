...
(() => {
    ...
    function fitDrawerButtonLabels(drawer) {
        drawer
            .querySelectorAll("button:not([data-live-close])")
            .forEach((button) => {
                button.style.removeProperty("font-size");
                let size = Number.parseFloat(
    ...
    function ensureUi() {
        ...
        drawer.hidden = true;
        drawer.innerHTML = `
            <header><div><small>PYBLOCKS</small><h2>Live Edit</h2></div><button class="live-edit-close-btn" type="button" data-live-close aria-label="Close Live Edit" title="Close Live Edit">&times;</button></header>
