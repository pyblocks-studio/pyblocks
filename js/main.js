document.addEventListener("DOMContentLoaded", () => {
    try {
        ...
        const theme = Blockly.Theme.defineTheme("pyblocks", {
            ...
            blockStyles: {
                event_blocks: {
                    colourPrimary: "#e0af68",
                    colourSecondary: "#c99a56",
                    colourTertiary: "#ad8143",
                    hat: "cap",
                },
                input_blocks: {
                    colourPrimary: "#7aa2f7",
                    colourSecondary: "#668ddf",
                    colourTertiary: "#5075c4",
                },
                misc_blocks: {
                    colourPrimary: "#737aa2",
                    colourSecondary: "#626987",
                    colourTertiary: "#515770",
                },
                logic_blocks: {
                    colourPrimary: "#73daca",
                    colourSecondary: "#63c5b7",
                    colourTertiary: "#4ca597",
                },
                loop_blocks: {
                    colourPrimary: "#7aa2f7",
                    colourSecondary: "#668ddf",
                ...
                text_blocks: {
                    colourPrimary: "#bb9af7",
                    colourSecondary: "#a681df",
                    colourTertiary: "#8e6ac6",
                },
                variable_blocks: {
                    colourPrimary: "#ff9e64",
                    colourSecondary: "#e88951",
                    colourTertiary: "#cc733e",
                },
                ...
                output_blocks: {
                    colourPrimary: "#9d7cd8",
                    colourSecondary: "#8968c2",
                    colourTertiary: "#7453aa",
                },
                list_blocks: {
                    colourPrimary: "#2ac3de",
                    colourSecondary: "#22abc4",
                    colourTertiary: "#188b9f",
                },
                procedure_blocks: {
                    colourPrimary: "#f7768e",
                    colourSecondary: "#d95f78",
                    colourTertiary: "#b94760",
                    hat: "cap",
                },
            },
            categoryStyles: {
        ...
        const updateCode = (event) => {
            ...
            if (!event || !event.isUiEvent) {
                ...
                validateLibraryBlocks(workspace);
            }
            if (
                event &&
                [
                ...
                synchronizeFunctionCalls(workspace);
        ...
        if ("ResizeObserver" in window) {
            new ResizeObserver(() => Blockly.svgResize(workspace)).observe(
...
function showStartupFailure(error) {
    console.error("PyBlocks editor startup failed.", error);
    ...
    message.textContent =
        "Reload the page. If the problem continues, verify that all vendored Blockly files are present.";
    workspace.replaceChildren(heading, message);
    workspace.classList.add("startup-error");
}
...
async function configureWorkspaceAudio(workspace) {
    ...
    audio.play = (soundName, requestedVolume = 1) => {
        if (window.PyBlocksSettings?.get().audio === false) return undefined;
...
function initProjectMenu(workspace) {
    ...
    document.getElementById("new-project-btn").addEventListener("click", () => {
        setMenuOpen(false);
        if (confirmReplacement("Starting a new project"))
            window.PythonEngine.newProject();
    });
    document
    ...
    projectInput.addEventListener("change", async () => {
        ...
        if (file.size > window.PyBlocksProjectFormat.MAX_FILE_BYTES) {
            window.PythonEngine.showError(
                "Project exceeds the 5 MB size limit.",
            );
            return;
        }
        try {
            const project = window.PyBlocksProjectFormat.parse(
                await file.text(),
            );
    ...
    document.getElementById("clear-btn").addEventListener("click", () => {
        ...
        if (
            confirmReplacement("Clearing the workspace") &&
            window.confirm("Clear every block from the workspace?")
        ) {
            workspace.clear();
        }
    });
...
function registerFunctionCategory(workspace) {
    workspace.registerToolboxCategoryCallback("PY_FUNCTIONS", () => {
        ...
        definitions.forEach((block) => {
            const functionName = (
                block.getFieldValue("NAME") || "function_name"
            )
                .replace(/[^\w]/g, "_")
                .replace(/^(\d)/, "_$1");
            ...
            const parameters = block.getParameterNames_
                ? block.getParameterNames_()
                : [];
            ...
            mutation.setAttribute("definition", block.id);
            call.appendChild(mutation);
            nodes.push(call);
            const statementCall = call.cloneNode(true);
            statementCall.setAttribute("type", "py_function_call_statement");
            nodes.push(statementCall);
        });
        return nodes;
    });
}

function synchronizeFunctionCalls(workspace) {
    ...
    [
        ...workspace.getBlocksByType("py_function_call", false),
        ...workspace.getBlocksByType("py_function_call_statement", false),
    ].forEach((call) => {
        const definition = definitions.get(call.definitionId_);
        if (definition) call.syncFromDefinition(definition);
    });
}
...
function initLibrariesDialog(workspace) {
    ...
    const modules = [
        ...
        ["datetime", "Export only: dates, times, durations, and formatting."],
        ["json", "Export only: read and write JSON-formatted data."],
        [
            "os",
            "Export only: operating-system paths, files, and environment access.",
        ],
        ["sys", "Export only: desktop Python runtime information."],
        ["statistics", "Export only: mean, median, mode, and statistics."],
        ["re", "Export only: regular-expression pattern matching."],
        ["pathlib", "Export only: object-oriented filesystem paths."],
        [
            "collections",
            "Export only: counters, default dictionaries, and deques.",
        ],
        ["itertools", "Export only: iterator building blocks."],
        [
            "functools",
            "Export only: function tools such as reduce and caching.",
        ],
    ...
    document.getElementById("libraries-btn").addEventListener("click", () => {
        const opener = document.getElementById("libraries-btn");
    ...
    document
        .getElementById("libraries-close-btn")
        .addEventListener("click", close);
    document
        .getElementById("libraries-done-btn")
        .addEventListener("click", close);
...
function validateLibraryBlocks(workspace) {
    const selected = window.PythonEngine.selectedLibraries;
    [
        ...workspace.getBlocksByType("py_library_call", false),
        ...workspace.getBlocksByType("py_library_call_statement", false),
    ].forEach((block) => {
        const moduleName = (block.getFieldValue("MODULE") || "").trim();
        block.setWarningText(
            selected.has(moduleName)
                ? null
                : `Enable “${moduleName}” in Libraries before running this call.`,
        );
    });
}
...
function updateLibraryToolbox(workspace, selected, modules) {
    ...
    selected.forEach((moduleName, index) => {
        ...
        category.setAttribute(
            "colour",
            ["#7dcfff", "#9ece6a", "#e0af68", "#f7768e", "#bb9af7"][index % 5],
        );
        ...
        functionField.textContent =
            metadata?.[0] === "time" ? "sleep" : "function";
        genericCall.append(moduleField, functionField);
        category.appendChild(genericCall);
        const genericStatement = genericCall.cloneNode(true);
        genericStatement.setAttribute("type", "py_library_call_statement");
        category.appendChild(genericStatement);
